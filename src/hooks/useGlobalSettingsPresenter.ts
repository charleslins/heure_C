...
export function useGlobalSettingsPresenter() {
  ...
  const presenter = useMemo(
    () => new GlobalSettingsPresenter(collection),
    [collection],
  );

  // Estado para armazenar os dados processados
  const [viewModel, setViewModel] =
    useState<ReturnType<typeof presenter.getSettingsViewModel>>();
  ...
  const handleUpdateWorkTime = async (data: WorkTimeDefaults) => {
    ...
    if (result.success) {
      ...
      addNotification(
        result.message || t("globalSettings.workTimeUpdateError"),
        "error",
      );
    }
    return result;
  ...
  const handleUpdateVacationRules = async (data: VacationRules) => {
    ...
    if (result.success) {
      addNotification(
        t("globalSettings.vacationRulesUpdateSuccess"),
        "success",
      );
      setViewModel(presenter.getSettingsViewModel());
    } else {
      addNotification(
        result.message || t("globalSettings.vacationRulesUpdateError"),
        "error",
      );
    }
    return result;
  ...
  const handleUpdateRegionalSettings = async (data: RegionalSettings) => {
    ...
    if (result.success) {
      addNotification(
        t("globalSettings.regionalSettingsUpdateSuccess"),
        "success",
      );
      setViewModel(presenter.getSettingsViewModel());
    } else {
      addNotification(
        result.message || t("globalSettings.regionalSettingsUpdateError"),
        "error",
      );
    }
    return result;
  ...
  const handleUpdateGeneralSettings = async (data: {
    ...
    if (result.success) {
      addNotification(
        t("globalSettings.generalSettingsUpdateSuccess"),
        "success",
      );
      setViewModel(presenter.getSettingsViewModel());
    } else {
      addNotification(
        result.message || t("globalSettings.generalSettingsUpdateError"),
        "error",
      );
    }
    return result;
  ...
  const handleResetToDefaults = async () => {
    ...
    if (result.success) {
      ...
      addNotification(
        result.message || t("globalSettings.resetError"),
        "error",
      );
    }
    return result;
  ...
  return {
    ...
    getWorkHoursForDate: (date: Date) => presenter.getWorkHoursForDate(date),
