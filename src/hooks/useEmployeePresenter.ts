...
export function useEmployeePresenter() {
  ...
  const presenter = useMemo(
    () => new EmployeePresenter(collection),
    [collection],
  );

  // Estado para armazenar os dados processados
  const [viewModel, setViewModel] =
    useState<ReturnType<typeof presenter.getEmployeeListViewModel>>();
  ...
  const handleCreateEmployee = async (data: Partial<Employee>) => {
    ...
    if (result.success) {
      ...
      addNotification(
        result.message || t("employeeManagement.employeeRegistration.error"),
        "error",
      );
    }
    return result;
  ...
  const handleUpdateEmployee = async (
    ...
    if (result.success) {
      ...
      addNotification(
        result.message || t("employeeManagement.updateError"),
        "error",
      );
    }
    return result;
  ...
  const handleRemoveEmployee = async (employeeId: string) => {
    ...
    if (result.success) {
      ...
      addNotification(
        result.message || t("employeeManagement.removeError"),
        "error",
      );
    }
    return result;
