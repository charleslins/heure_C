...
export const saveGlobalHolidaysToSupabase = async (
  ...
  if (fetchError) {
    ...
    return {
      success: false,
      error: fetchError,
      message: "Error fetching existing holiday IDs.",
    };
  }
  ...
  const holidaysToUpsert = holidays.map((holiday) => {
    ...
    const isTemporaryOrMockId =
      originalClientId &&
      (originalClientId.startsWith("holiday-") ||
        originalClientId.startsWith("gh-"));
...
export const loadUserWeeklyContractFromSupabase = async (
  userId: string,
  year: number,
  month: number,
...
export const saveUserWeeklyContractToSupabase = async (
  userId: string,
  year: number,
  month: number,
...
// --- User Vacations ---
export const loadUserVacationsForMonthFromSupabase = async (
  userId: string,
  year: number,
  month: number,
...
export const saveUserVacationsForMonthToSupabase = async (
  userId: string,
  year: number,
  month: number,
...
export const loadTypedUserMonthDataFromSupabase = async <T>(
  ...
  switch (dataType) {
    case "vacations":
      return (await loadUserVacationsForMonthFromSupabase(
        userId,
        year,
        month,
