...
import { SEGMENT_COLORS, SEGMENT_HOVER_COLORS } from "@/utils/constants";
...
const DailyLogRow: React.FC<DailyLogRowProps> = ({
  logEntry,
  onLogEntryChange,
  userGlobalSettings,
  weeklyContract,
  ...
  const minHour = parseTimeToHour(
    userGlobalSettings?.workTimeDefaults?.overallStartTime,
  );
  const maxHour = parseTimeToHour(
    userGlobalSettings?.workTimeDefaults?.overallEndTime,
  );
  ...
  const handleTypeChange = (
    ...
    const shouldClearTimes =
      type === EntryType.VACANCE ||
      type === EntryType.FERIE ||
      type === EntryType.RECUPERATION ||
      type === EntryType.MALADIE;
    onLogEntryChange({
      ...logEntry,
      [period]: {
        ...logEntry[period],
  ...
  const dayOfWeekShort = logEntry.date
  ...
  const isMorningTimeInputDisabled = isTypeDisablingTimeInputs(
    logEntry.morning.type,
  );
  const isAfternoonTimeInputDisabled = isTypeDisablingTimeInputs(
    logEntry.afternoon.type,
  );

  // Determine if a segment should appear faded but remain enabled for exceptional input
  const isMorningVisuallyFaded =
    morningContractHours === 0 &&
    logEntry.morning.type === EntryType.REGULAR &&
    !logEntry.morning.start &&
    !logEntry.morning.end;
  const isAfternoonVisuallyFaded =
    afternoonContractHours === 0 &&
    logEntry.afternoon.type === EntryType.REGULAR &&
    !logEntry.afternoon.start &&
    !logEntry.afternoon.end;

  const rowLevelFaded =
    (!logEntry.isWorkingDay || isWeekend) && !logEntry.hasInputs;
  ...
  const getSegmentClasses = (
    ...
    const isSegmentTypeNonWorkingOverride =
      segmentData.type === EntryType.VACANCE ||
      segmentData.type === EntryType.FERIE ||
      segmentData.type === EntryType.RECUPERATION ||
      segmentData.type === EntryType.MALADIE;
    const isSegmentEmptyAndRegular =
      !segmentData.start &&
      !segmentData.end &&
      segmentData.type === EntryType.REGULAR;
    ...
    if (rowLevelFaded && isSegmentEmptyAndRegular) {
      ...
      classes +=
        (SEGMENT_COLORS[segmentData.type] || "bg-white border-slate-200") + " ";
    }
  ...
  return (
    <tr className={rowClasses}>
      <td
        className={`px-3 py-1.5 text-sm text-slate-700 whitespace-nowrap align-middle ${rowLevelFaded ? "" : ""}`}
      >
        <div className="font-medium text-center">
          {logEntry.date.toLocaleDateString(i18n.language, {
            day: "2-digit",
            month: "2-digit",
          })}
        </div>
        <div className="text-xs text-slate-500 capitalize text-center min-w-[45px] tracking-wide">
          {dayOfWeekShort}
        </div>
      </td>
      ...
      <td
        className={getSegmentClasses(
          logEntry.morning,
          morningContractHours,
          isMorningVisuallyFaded,
          true,
        )}
      >
        <TimeInput
      ...
      <td
        className={getSegmentClasses(
          logEntry.afternoon,
          afternoonContractHours,
          isAfternoonVisuallyFaded,
          true,
        )}
      >
        <TimeInput
      ...
      {/* Total */}
      <td
        className={`px-3 py-1.5 text-sm font-bold text-slate-800 text-right whitespace-nowrap align-middle ${rowLevelFaded ? "" : ""}`}
      >
