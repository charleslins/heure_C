...
const EmployeeRegistrationForm: React.FC<EmployeeRegistrationFormProps> = ({
  ...
  const handleInputChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        name === "canton_id" || name === "municipality_id"
          ? value
            ? parseInt(value)
            : undefined
          : value,
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = t(
        "employeeManagement.employeeRegistration.errors.nameRequired",
      );
    }

    if (!formData.email.trim()) {
      newErrors.email = t(
        "employeeManagement.employeeRegistration.errors.emailRequired",
      );
    } else if (!/\S+@\S+\.\S+/.test(formData.email)) {
      newErrors.email = t(
        "employeeManagement.employeeRegistration.errors.emailInvalid",
      );
    }
    ...
    if (!isEditMode && (!formData.password || formData.password.length < 6)) {
      newErrors.password = t(
        "employeeManagement.employeeRegistration.errors.passwordMinLength",
      );
    }

    if (!formData.canton_id) {
      newErrors.canton_id = t(
        "employeeManagement.employeeRegistration.errors.cantonRequired",
      );
    }

    if (!formData.position?.trim()) {
      newErrors.position = t(
        "employeeManagement.employeeRegistration.errors.positionRequired",
      );
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && employee) {
        ...
        if (result.success) {
          addNotification(
            t("employeeManagement.employeeRegistration.success"),
            "success",
          );
          if (onSuccess) {
            onSuccess();
          ...
          addNotification(
            result.message ||
              t("employeeManagement.employeeRegistration.error"),
            "error",
          );
        }
      }
    ...
    return (
      <div className="flex items-center justify-center p-8">
        ...
        <span className="ml-2 text-gray-600">
          {t("employeeManagement.loadingData")}
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">
        {isEditMode
          ? t("employeeManagement.employeeRegistration.editTitle")
          : t("employeeManagement.employeeRegistration.title")}
      </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder={t(
              "employeeManagement.employeeRegistration.namePlaceholder",
            )}
          />
          {errors.name && (
        ...
        <div>
          ...
          <input
            ...
            placeholder={t(
              "employeeManagement.employeeRegistration.emailPlaceholder",
            )}
          />
          {errors.email && (
        ...
        <div>
          ...
          {errors.canton_id && (
            <p className="text-red-500 text-xs mt-1">{errors.canton_id}</p>
          )}
        </div>
          ...
          >
            ...
            {municipalities.map((municipality) => (
              <option key={municipality.id} value={municipality.id}>
                {municipality.name}{" "}
                {municipality.postal_code && `(${municipality.postal_code})`}
              </option>
            ))}
          </select>
        </div>
