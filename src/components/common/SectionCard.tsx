...
const SectionCard: React.FC<SectionCardProps> = ({
  ...
  return (
    <div className={`${cardClassName}`}>
      {hasHeader && (
        <div
          className={`flex justify-between items-center ${headerAreaClassName}`}
        >
          {title && (
            <h2 className={`flex items-center ${titleTextClassName}`}>
              {TitleIcon && titleIconProps && <TitleIcon {...titleIconProps} />}
              <span className={TitleIcon && titleIconProps ? "ml-2" : ""}>
                {title}
              </span>
            </h2>
          )}
          {headerActions && (
            <div className="ml-auto pl-4 flex-shrink-0">{headerActions}</div>
          )}
        </div>
      )}
