...
import { VACATION_STATUS_STYLES } from "@/utils/constants";
...
const CalendarGrid: React.FC<CalendarGridProps> = ({
  ...
  const daysOffset = useMemo(
    () => getFirstDayOfMonthGridOffset(year, month),
    [year, month],
  );
  const dayHeaders = useMemo(
    () => generateDayHeaders(i18n.language),
    [i18n.language],
  );

  const handleDayClickInternal = (dateStr: string) => {
    const dayDate = new Date(dateStr + "T00:00:00");
    const isGlobalHolidayOnClick = globalHolidays.some(
      (h) => h.date === dateStr,
    );
    if (isGlobalHolidayOnClick) return;

    const logEntryForDay = dailyLogs.find(
      (log) => formatDateToYYYYMMDD(log.date) === dateStr,
    );
    if (
      logEntryForDay &&
      (logEntryForDay.morning.type === EntryType.FERIE ||
        logEntryForDay.afternoon.type === EntryType.FERIE)
    )
      return;
    if (
      logEntryForDay &&
      (logEntryForDay.morning.type === EntryType.RECUPERATION ||
        logEntryForDay.afternoon.type === EntryType.RECUPERATION)
    )
      return;
    if (
      logEntryForDay &&
      (logEntryForDay.morning.type === EntryType.MALADIE ||
        logEntryForDay.afternoon.type === EntryType.MALADIE)
    )
      return;
    ...
    <>
      <div className="grid grid-cols-7 gap-px text-center text-xs sm:text-sm font-medium text-slate-600 bg-slate-200 rounded-t-md">
        {dayHeaders.map((header) => (
          <div key={header} className="py-1.5 px-1">
            {header}
          </div>
        ))}
      </div>
      <div className="grid grid-cols-7 gap-2 bg-white rounded-2xl p-2">
        {Array.from({ length: daysOffset }).map((_, i) => (
          <div
            key={`empty-start-${i}`}
            className="bg-slate-50 h-16 w-16 sm:h-20 sm:w-20 p-2 rounded-lg"
          ></div>
        ))}
        {daysInCalendarMonth.map((dayDate) => {
          ...
          const userVacation = vacationsForMonth.find(
            (v) => v.date === dateStr,
          );
          ...
          if (isGlobalHoliday) {
            ...
            indicatorElement = (
              <div className="mt-0.5">
                <span className="text-xxs sm:text-xs font-medium text-yellow-700">
                  {t("vacationPage.holidayLabel")}
                </span>
              </div>
            );
            ...
            indicatorElement = (
              <div className="mt-0.5">
                <span className="text-xxs sm:text-xs font-medium text-cyan-700">
                  {t("vacationPage.recuperationLabel", "RECUP.")}
                </span>
              </div>
            );
            indicatorTextForAria = t(
              "vacationPage.recuperationLabel",
              "RECUP.",
            );
          } else if (isSickDay) {
            ...
            indicatorElement = (
              <div className="mt-0.5">
                <span className="text-xxs sm:text-xs font-medium text-red-700">
                  {t("vacationPage.sickLeaveLabel", "ATEST.")}
                </span>
              </div>
            );
            ...
            indicatorElement = (
              <div className="mt-0.5">
                <span
                  className={`text-xxs sm:text-xs font-medium ${statusStyle.text}`}
                >
                  {t(`vacationStatuses.${userVacation.status}`)}
                </span>
              </div>
            );
            indicatorTextForAria = t(`vacationStatuses.${userVacation.status}`);
            isActionable = userVacation.status !== VacationStatus.APPROVED;
            if (userVacation.status === VacationStatus.APPROVED)
              cellBgClass += " opacity-70";
            if (userVacation.status === VacationStatus.PENDING_APPROVAL)
              cellBgClass += " opacity-80";
          } else if (isWorkingDay) {
            cellBgClass =
              "bg-white hover:bg-indigo-50 focus-within:bg-indigo-50";
            indicatorElement = (
              <div className="mt-0.5">
                <span className="text-xxs sm:text-xs text-indigo-700 font-medium">
                  {formatHours(totalContractHours)}
                  {t("common.hoursUnitShort")}
                </span>
              </div>
            );
          ...
          return (
            <div
              ...
              `}
              onClick={
                isActionable ? () => handleDayClickInternal(dateStr) : undefined
              }
              onKeyDown={
                isActionable
                  ? (e) =>
                      (e.key === "Enter" || e.key === " ") &&
                      handleDayClickInternal(dateStr)
                  : undefined
              }
        ...
        {Array.from({
          length: (7 - ((daysInCalendarMonth.length + daysOffset) % 7)) % 7,
        }).map((_, i) => (
          <div
            key={`empty-end-${i}`}
            className="bg-slate-50 h-16 w-16 sm:h-20 sm:w-20 p-2 rounded-lg"
