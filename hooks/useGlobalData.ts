...
export const useGlobalData = () => {
  const [globalUserSettings, setGlobalUserSettings] = useState<UserGlobalSettings>(INITIAL_USER_GLOBAL_SETTINGS);
  const [globalHolidays, setGlobalHolidays] = useState<Holiday[]>(MOCK_GLOBAL_HOLIDAYS);
  ...
  const fetchAllGlobalData = useCallback(async () => {
    ...
    try {
      ...
      const loadedHolidays = await loadGlobalHolidaysFromSupabase(MOCK_GLOBAL_HOLIDAYS);
      ...
      if (usersFromDb.length > 0) {
        ...
        console.warn("No users found in Supabase 'profiles' table, falling back to mock users.");
