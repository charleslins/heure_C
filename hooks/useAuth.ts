  ...
  const processAndSetUser = useCallback(async (sessionUser: any | null) => {
    ...
    try {
      ...
      if (profileError && profileError.code !== 'PGRST116') { // PGRST116: row not found
        console.warn('Error fetching user profile:', profileError.message, '- falling back to session metadata.');
        const nameFromMeta = sessionUser.user_metadata?.name || sessionUser.email || 'User';
        appUser = {
          id: sessionUser.id,
        ...
        const profileName = profileData?.name || sessionUser.user_metadata?.name;
        ...
        appUser = {
          ...
          photoUrl: profileData?.photo_url || sessionUser.user_metadata?.photoUrl,
  ...
  useEffect(() => {
    ...
    const checkInitialSession = async () => {
      try {
        ...
        if (initialSessionError) {
          console.error("Error fetching initial session:", initialSessionError.message);
        }
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        if (event === 'INITIAL_SESSION' || event === 'SIGNED_IN' || event === 'SIGNED_OUT' || event === 'USER_DELETED' || event === 'PASSWORD_RECOVERY') {
             setIsLoadingAuth(false);
        }
      }
    );
