  ...
  try {
    ...
    if (employeeData.employee_id && employeeData.employee_id.trim() !== '') {
      ...
      if (checkError && checkError.code !== 'PGRST116') {
        ...
        return { success: false, error: checkError, message: 'Erro ao verificar ID do funcionário' };
      }
    ...
    const cleanEmployeeId = employeeData.employee_id && employeeData.employee_id.trim() !== '' 
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '23505' && error.message.includes('employee_id_key')) {
        return { 
          success: false, 
          message: 'Este ID de funcionário já está em uso. Escolha um ID diferente ou deixe em branco.' 
        };
      }
    ...
    return { success: false, error, message: 'Erro inesperado ao atualizar funcionário' };
  }
};
