...
const StyledSummaryItem: React.FC<StyledSummaryItemProps> = ({
  label, value, unit, icon, itemBgClass, accentTextClass, accentBorderClass
}) => (
  <div className={`p-3 rounded-lg shadow-md flex items-center space-x-3 ${itemBgClass} ${accentBorderClass} border-l-4 h-full`}>
...
const ContractAndMonthlySummaryCard: React.FC<ContractAndMonthlySummaryCardProps> = ({ userGlobalSettings, summaryData }) => {
  const { t } = useTranslation();
  const contractedWeeklyHours = summaryData.contractedWeeklyHours || 0;
  
  const calculatedTaux = STANDARD_FULL_TIME_HOURS_WEEKLY > 0 
    ? parseFloat(((contractedWeeklyHours / STANDARD_FULL_TIME_HOURS_WEEKLY) * 100).toFixed(1)) 
    : 0;

  const baseAnnualVacationDays = userGlobalSettings.annualVacationDays || 0;
  const actualAnnualVacationDays = parseFloat(((calculatedTaux / 100) * baseAnnualVacationDays).toFixed(1));
